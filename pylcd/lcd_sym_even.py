...
class LCDSymmEvenOptimizer:
    ...
    def get_samples(self, n_samples_half, initial_samples=None):
        ...
        return np.reshape(optimization_result.x, (-1, self.dim))
...
if __name__ == "__main__":
    ...
    optimized_s_vectors = lcd_opt.get_samples(n_samples_half_test).reshape(-1, dim_test)
    print("Optimized s_vectors:", optimized_s_vectors)

    symm_samples = np.vstack((optimized_s_vectors, -optimized_s_vectors))

    test_mean = np.mean(symm_samples, axis=0)
    print("Mean of symmetrized samples:", test_mean)

    test_cov = np.cov(symm_samples, rowvar=False)
    print("Cov of symmetrized samples:", test_cov)

    cov_chol = np.linalg.cholesky(test_cov)

    standardized_symm_samples = np.linalg.solve(cov_chol, optimized_s_vectors.T).T
